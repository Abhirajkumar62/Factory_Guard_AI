...
class SensorSimulator:
    ...
    def __init__(self, n_arms: int = 500, api_endpoint: str = "http://localhost:5001"):
        ...
        self.failure_arms = set(random.sample(range(n_arms), max(1, n_arms // 100)))  # ~1% will fail
        
        self.logger.info(f"Initialized simulator for {n_arms} arms")
        self.logger.info(f"⚠️ {len(self.failure_arms)} arms scheduled to fail in test scenario")
    ...
    def generate_healthy_readings(self) -> Dict:
        ...
        return {
            ...
            'power_consumption': self.baseline['power_consumption'] + np.random.normal(0, 20)
        }
    ...
    def generate_degrading_readings(self, time_step: float) -> Dict:
        ...
        return {
            'temperature': self.baseline['temperature'] + temp_increase + np.random.normal(0, 1),
            'pressure': self.baseline['pressure'] + pressure_increase + np.random.normal(0, 10),
            'vibration': self.baseline['vibration'] + vibration_increase + np.random.normal(0, 0.2),
            'humidity': self.baseline['humidity'] + np.random.normal(0, 5),
            'power_consumption': self.baseline['power_consumption'] + power_increase + np.random.normal(0, 30)
        }
    
    def generate_arm_readings(self, arm_index: int, time_since_start: float = 0) -> Dict:
        """
    ...
    def generate_fleet_readings(self, time_since_start: float = 0, sample_size: int = None) -> List[Dict]:
        """
    ...
    def send_prediction_request(self, readings: Dict) -> Dict:
        ...
        try:
            response = requests.post(
                f"{self.api_endpoint}/predict/arm",
                json=readings,
                timeout=5
            )
    ...
    def run_simulation(self, duration_hours: int = 24, interval_minutes: int = 5,
                     sample_size: int = 100, verbose: bool = True) -> List[Dict]:
        """
        ...
        while elapsed_hours < duration_hours:
            ...
            if result:
                ...
                if alerts and verbose:
                    self.logger.warning(f"🚨 {len(alerts)} ALERTS at hour {elapsed_hours:.1f}:")
        ...
        self.logger.info(f"✅ Simulation complete. {len(predictions)} prediction cycles recorded")
        return predictions
