...
class FailurePredictionEngine:
    ...
    def __init__(self, model_path: str = "models/xgboost_model.pkl",
        ...
        self.logger.info(f"Initialized prediction engine (threshold: {failure_threshold})")
    ...
    def analyze_arm_health(self, arm_id: str, current_features: np.ndarray,
    ...
    def _assess_trends(self, trends: Dict) -> Dict:
        ...
        return {
            ...
            'warnings': warnings,
            'trend_score': float(max(0, 1 - (len(warnings) * 0.15)))  # Penalty for each warning
        }
    ...
    def generate_maintenance_report(self, fleet_assessments: List[Dict]) -> Dict:
        ...
        report = {
            ...
            'immediate_action_required': len([a for a in alerts if a['maintenance_priority'] == 'immediate']),
            'urgent_maintenance': len([a for a in alerts if a['maintenance_priority'] == 'urgent']),
            ...
            'average_fleet_health': float(np.mean([a['health_score'] for a in fleet_assessments]))
        }
